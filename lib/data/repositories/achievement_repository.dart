...
class AchievementRepository {
  ...
  Future<List<Achievement>> getAllAchievements() async {
    final List<AchievementsTableData> rows = await database.getAllAchievements();
  ...
  Future<void> unlockAchievement(String key) async {
    ...
    await database.updateAchievement(AchievementsTableCompanion(
      id: Value(row.id),
      key: Value(row.key),
      name: Value(row.name),
  ...
  Future<void> seedAchievements(List<Achievement> achievements) async {
    final List<AchievementsTableData> existing =
        await database.getAllAchievements();
    final Set<String> existingKeys = existing.map((a) => a.key).toSet();
    for (final achievement in achievements) {
      if (!existingKeys.contains(achievement.key)) {
        await database.insertAchievement(AchievementsTableCompanion(
